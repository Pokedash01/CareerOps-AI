...
# How long a job "counts" as already-seen before the pipeline will look at
# it again. Without this, a static/narrow query pool gets permanently
# exhausted within a couple of runs and the pipeline goes silent forever.
...
def load_state() -> dict:
    ...
    # Migrate old format: seen_jobs used to be a flat list with no
    # timestamps, which meant a job was skipped forever once evaluated.
    ...
    # Separate, permanent record of jobs already pushed to the user via
    # Telegram - this one never expires, so TTL-driven re-evaluation can't
    # cause the same posting to be sent twice.
